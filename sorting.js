...
window.onload = () => {
    ...
    const sortOrder =
        document.getElementsByClassName("select-selected")[0].firstChild;
...
function handleArraySize() {
    ...
    document.getElementById(
        "comp"
    ).innerHTML = `Number Of Comparisons : ${comp}`;
    swap = 0;
    ...
    const inputOrder =
        document.getElementsByClassName("select-selected")[1].firstChild;
...
function handleSort() {
    ...
    const speedOrder =
        document.getElementsByClassName("select-selected")[2].firstChild;
    ...
    for (let i = 0; i < size; i++) {
        ...
        for (let j = i + 1; j < size; j++) {
            ...
            if (val1 < val2) {
                ...
                min_ind = j;
            } else bars[j].style.backgroundColor = "violet";
        }
        if (min_ind != i) {
            swap++;
            document.getElementById(
                "swap"
            ).innerHTML = `Number Of Swaps : ${swap}`;
        }
...
async function partition(left, right, delay) {
    ...
    do {
        ...
        while (
            i <= right &&
            Number.parseInt(bars[i].childNodes[0].innerHTML) <= pivot
        ) {
            flag = false;
        ...
        while (
            j >= left &&
            Number.parseInt(bars[j].childNodes[0].innerHTML) > pivot
        ) {
            flag = false;
    ...
    for (let k = left; k <= right; k++)
        bars[k].style.backgroundColor = "violet";
